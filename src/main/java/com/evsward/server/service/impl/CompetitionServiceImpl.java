...
public class CompetitionServiceImpl extends BaseServiceImpl<CompetitionInfo, Integer> implements CompetitionService {
	...
	public CompManPlayerInfo getLivedPlayersByComp(int compID) throws Exception {
		...
		CompManPlayerInfo playersInfo = new CompManPlayerInfo(totalRegedPlayer, totalRegedPlayerEdit, totalChip,
				totalChipEdit, list);
		return playersInfo;
	}
	...
	public Map<String, Object> getCompMemPrizeInfoList(int compID) throws Exception {
		...
		// 计算总奖金
		long totalPrize = HIUtil.calcCompTotalPrizeMoney(compInfo.getLeastPrize(), compInfo.getRegFee(),
				compInfo.getTotalPlayer());
		// 减去扣除人数后，计算的总奖金
		long totalPrizeEdit = HIUtil.calcCompTotalPrizeMoney(compInfo.getLeastPrize(), compInfo.getRegFee(),
				compInfo.getTotalPlayer(), compInfo.getSubPlayerCount());
		...
		List<CompRunningPrize> compRunningPrizePlayerList = this.runningPrizeDao
				.getExistCompRunningPrizeListWithMemInfo(compID);
	...
	public void updateOpenRegistration(int compID, int[] tableNOArr) throws Exception {
		...
		if (compInfo.getAssignSeat() == CompetitionInfo.ASSIGNSEATSTATE.USE) {// 使用分桌系统
			...
			List<UnAllotedSeat> insertList = HIUtil.creatRandomSeat(compID, compInfo.getTableType(),
					compInfo.getSysType(), cardTableList);
	...
	public int updateOutMemFromComp(int cmID, int memID, int compID) throws Exception {
		...
		if (compInfo.getAword() == CompetitionInfo.AWORDSTATE.WITHAWORD
			...
			// 比赛总奖金
			int compTotalPrize = HIUtil.calcCompTotalPrizeMoney(compInfo.getLeastPrize(), compInfo.getRegFee(),
					compInfo.getTotalPlayer(), compInfo.getSubPlayerCount());
			/**
			 * 产生奖励记录
			 */
		...
		this.seatUnAllotDao.addUnAllotedSeatOfComp(new UnAllotedSeat(cm.getTableNO(), cm.getSeatNO(), cm.getCompID(),
				new Date().getTime(), HIUtil.getSeatLevel(cm.getSeatNO(), compInfo.getTableType()),
				compInfo.getTableType(), compInfo.getSysType()));
	...
	public int updateGoForwardRound(int compID) throws Exception {
		...
		if (runningRound.getCurRank() == compInfo.getStopRegRank()
	...
	public int updateRoundJumpTime(int compID, int curRank, int jumpTime) throws Exception {
		...
		if (runningRound == null) {
			...
			logger.error(
					"客户端盲注级别不同步，客户端rank=" + curRank + ",服务端，rank=" + runningRound.getCurRank() + ",compID=" + compID);
			return -3;
		}
	 ...
	 * @param origCompList
	 *            源比赛集合
	 * @param orignalTablesList
	 *            源比赛牌桌集合（这个参数是固定分座使用的）
	 * @param uniqueCMMap
	 *            排重后的导入选手
	 * @param outCMList
	 *            淘汰的选手
	 ...
	 */
	@Transactional(propagation = Propagation.REQUIRED, isolation = Isolation.DEFAULT)
	public int updateImportComps(List<CompetitionInfo> origCompList, List<Integer> orignalTablesList,
			Map<Integer, CompetitionMember> uniqueCMMap, List<CompetitionMember> outCMList, CompetitionInfo destComp,
			List<CardTable> usedTables, int condition, String empUuid) throws Exception {
		...
		for (int i = 0; i < origCompList.size(); i++) {
			...
			// 添加日志记录
			this.compImportLogDao
					.insert(new CompImportLog(compInfo.getCompID(), destComp.getCompID(), importTime, empUuid));
		}
		...
		if (destComp.getAword() == CompetitionInfo.AWORDSTATE.WITHAWORD && outCMList != null && outCMList.size() > 0) {
			...
			double percent = 0.0d;
			int compTotalPrize = HIUtil.calcCompTotalPrizeMoney(destComp.getLeastPrize(), destComp.getRegFee(),
					importTotalPlayer, importTotalSubPlayer);// (importTotalPlayer
																// -
																// importTotalSubPlayer)
																// *
																// destComp.getRegFee();
			double amount = 0.0d;
			int amountInt = 0;
		...
		for (int i = 0; i < outCMList.size(); i++) {
			...
			newOutCM = new CompetitionMember(outCM.getMemID(), destComp.getCompID(), CompetitionMember.MemCompState.OUT,
					-1, -1, 0, destComp.getSysType(), regTime);
	...
	public CompBurstTableRes updateburstTableFromComp(int compID, int tableNO, String empUuid) throws Exception {
		...
		// 根据存活选手数量随机找新座位
		List<UnAllotedSeat> newSeatList = this.seatUnAllotDao.getUnAllotedSeatByCompID(compID, tableNO,
				livedMemCompList.size());
		...
		for (int i = 0; i < newSeatList.size(); i++) {
			...
			// 插入到爆桌记录表
			this.compHisLogDao.insert(new CompMovedSeatLog(compID, cm.getMemID(), cm.getTableNO(), cm.getSeatNO(),
					empUuid, newSeat.getTableNO(), newSeat.getSeatNO(), CompMovedSeatLog.OPTYPE.BURST, date.getTime(),
					compInfo.getSysType()));
			memInfo = new MemberInfo();
			memInfo.setMemID(cm.getMemID());
			memInfo = this.memDao.getExactMemInfo(memInfo);
			burstTableNewSeatInfo = new BurstTableNewSeatInfo(memInfo.getMemID(), memInfo.getCardNO(),
					memInfo.getMemName(), newSeat.getTableNO(), newSeat.getSeatNO(), memInfo.getMemSex());
		...
		if (compMemInfoList != null && !compMemInfoList.isEmpty()) {
			for (int i = 0; i < compMemInfoList.size(); i++) {
				...
				screenCompPlayerInfo = new ScreenCompPlayerInfo(i + 1, compMemInfo.getCardNO(),
						compMemInfo.getMemName(), compMemInfo.getTableNO(), compMemInfo.getSeatNO(),
						compMemInfo.getChip());
	...
	public List<ScreenCompInfo> getScreenCompList(Date start, Date end, int sysType) throws Exception {
		...
		if (list != null && list.size() > 0) {
			for (int i = 0; i < list.size(); i++) {
				...
				screenCompInfo = new ScreenCompInfo(info.getCompID(), info.getCompName(), info.getCompState(),
						info.getBeginChip(), DateUtils.formatDate(info.getStartTime(), DateUtils.PATTERN2),
						DateUtils.formatDate(info.getStartTime(), DateUtils.PATTERN7),
						info.getTotalPlayer() - info.getSubPlayerCount(), curRank);
	...
	public int updateStartCompetition_Serv(CompetitionInfo compInfo, int newCompState, boolean flag) throws Exception {
		CompRunningRound rR = this.runningRoundDao.getRunningRoundByCompID(compInfo.getCompID());
		if (flag) {
			if (rR.getCurRank() == 1) {
	...
	public int updateCompRoundRun_Serv(CompetitionInfo compInfo) throws Exception {
		...
		if (deviation >= -500) {// 偏差值小于等于1秒，就可以进行下一个盲注了。
			round = this.roundDao.getNextRound_Serv(runningRound.getRoundTempID(), runningRound.getCurRank(),
					runningRound.getCurType());
			if (round != null && round.getRoundType() == Round.ROUNDTYPE.TIME) {
				if (runningRound.getCurRank() + 1 == compInfo.getStopRegRank()
						&& (compInfo.getCompState() == CompetitionInfo.COMPSTATE.STATE_REGING_BEGINED
								|| compInfo.getCompState() == CompetitionInfo.COMPSTATE.STATE_REGEND_NOBEGIN)) {// 下个级别就该截止报名了
					this.compDao.endCompetitionReg_Serv(compInfo.getCompID());
				}
			}
			logger.info("--------------盲注跳跃时间到--------------compID = " + compInfo.getCompID() + ", compName = "
					+ compInfo.getCompName());
			if (round != null) {
				...
				logger.info("--------------盲注已经跳尽--------------compID = " + compInfo.getCompID() + ", compName = "
						+ compInfo.getCompName());
				return -1;
			}
		}
	 ...
	 *            源比赛的桌号
	 * @param uniqueCMMap
	 *            排重后的选手<memID, CompetitionMember>
	 * @param usedTables
	 *            目标比赛占用的桌子集合
	 * @param destComp
	 *            目标比赛
	 ...
	 * @throws Exception
	 */
	private void allotSeatForImport(List<Integer> orignalTablesList, Map<Integer, CompetitionMember> uniqueCMMap,
			List<CardTable> usedTables, CompetitionInfo destComp, int condition) throws Exception {
		...
		if (condition != 1) {// 随机分座，获取目标赛的所有待分座位
			...
			while (it.hasNext()) {
				...
				newcm = new CompetitionMember(cm.getMemID(), destComp.getCompID(), CompetitionMember.MemCompState.REGED,
						unAllotedSeat.getTableNO(), unAllotedSeat.getSeatNO(), cm.getChip(), destComp.getSysType(),
						regTime);
				// 删除该分配过的座位
				int res = this.seatUnAllotDao.delSpecUnAllotedSeat(destComp.getCompID(), unAllotedSeat.getTableNO(),
						unAllotedSeat.getSeatNO());
				if (res <= 0) {
					throw new RuntimeException("晋级导入--随机座位，座位被占用");
				}
				// 插入已分座位表中
				this.seatAllotedDao.insertAllotedSeat(new AllotedSeat(unAllotedSeat.getTableNO(),
						unAllotedSeat.getSeatNO(), destComp.getCompID(), cm.getMemID(), regTime));
				// 生成报名数据
			...
			while (it.hasNext()) {
				...
				newcm = new CompetitionMember(cm.getMemID(), destComp.getCompID(), CompetitionMember.MemCompState.REGED,
						newTableNO, cm.getSeatNO(), cm.getChip(), destComp.getSysType(), regTime);
				...
				this.seatAllotedDao.insertAllotedSeat(
						new AllotedSeat(newTableNO, cm.getSeatNO(), destComp.getCompID(), cm.getMemID(), regTime));
				this.memCompDao.saveMemCompRelation(newcm);
			}
		}
	}
