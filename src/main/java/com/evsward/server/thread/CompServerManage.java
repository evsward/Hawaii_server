		...
		while(compInfo.getSysType() == Application.SYSTYPE.USECOMPMANAGE && 
			...
			if(!run.get()){
				if(System.currentTimeMillis() >= compStartTimeLong){//比赛时间到，比赛可以开始了。
					if(compInfo.getCompPause() == CompetitionInfo.PAUSESTATE.NOPAUSE){//比赛为暂停
						if(compInfo.getCompState() != CompetitionInfo.COMPSTATE.STATE_NOREG){
							run.set(true);
							try {
								if(compInfo.getCompState() == CompetitionInfo.COMPSTATE.STATE_REGING_NOBEGIN){//1、判断比赛状态，compState=1,则更新compState=2并且添加比赛当前运行盲注信息
									this.compService.updateStartCompetition_Serv(compInfo, CompetitionInfo.COMPSTATE.STATE_REGING_BEGINED);
									Application.setTriggingMsgForPushing(new SystemBasePushTriggMsg(Application.OPERTYPE.COMPTHREADSERV, 0, compID));
								}else if(compInfo.getCompState() == CompetitionInfo.COMPSTATE.STATE_REGEND_NOBEGIN){//比赛状态，compState=3,则更新compState=4并且添加比赛当前运行盲注信息
...
//				this.runningRound = this.runningRoundService.getCurRoundOfComp(compID);
			} catch (Exception e) {
				logger.error(e.getMessage(), e);
				return;
			}
