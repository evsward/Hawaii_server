...
//		Application.PATH_ROOT = context.getRealPath("/");
		...
		// 创建头像文件目录（如果存在就不创建）
		logger.info("Application.PATH_ROOT: " + Application.PATH_ROOT);
		String path = Application.PATH_ROOT + Application.PATH_IMAGE + System.getProperty("file.separator")
				+ Application.PATH_IMAGE_SMALL;
		logger.info("path: " + path);
		File imageDic = new File(path);
		if (!imageDic.exists()) {
			imageDic.mkdirs();
		}
