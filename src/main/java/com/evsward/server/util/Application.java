	...
	public static void startPushMsgServiceEntrance()throws Exception{
		...
		for (int i = 0; i < SystemCache.msgCmdList.size(); i++) {
			...
			if(!StringUtils.isEmpty(className)){
				PushMsg2TerminalService pushMsgServiceThread = (PushMsg2TerminalService)Class.forName(className).newInstance();
				pushMsg2TermServiceMap.put(SystemCache.msgCmdList.get(i).getMsgID(), pushMsgServiceThread);
				new Thread(pushMsgServThGroup, pushMsgServiceThread, SystemCache.msgCmdList.get(i).getMsgName()).start();
			}
		}
	}
